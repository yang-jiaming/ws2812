...
typedef struct ws2812bfx_s
{
  ...
  uint8_t ActualMode;       // 部门 mode setting
  uint8_t Reverse : 1;      // Is 恢复 mode
  uint32_t CounterModeCall; //通话次数
  uint32_t CounterModeStep; //呼叫步骤

  uint16_t Speed; //分段速度

  uint32_t ModeColor[NUM_COLORS];       //模式颜色32位表示法
  ws2812b_color ModeColor_w[NUM_COLORS]; //模式颜色结构表示法

  uint8_t AuxParam;     //计算变量
  uint16_t AuxParam16b; //计算变量
  uint8_t Cycle : 1;    //循环变量
...
FX_STATUS WS2812BFX_Init(uint16_t Segments)
{
  ...
  if (SegmentsTmp == NULL)
    return FX_ERROR; // If assigning failed
  ...
  if (Ws28b12b_Segments == NULL)
  {
    ...
    for (uint16_t i = 0; i < mSegments; i++)
    {
      ...
      if (SegmentsTmp[i].IdStop >= WS2812B_LEDS)
        SegmentsTmp[i].IdStop = WS2812B_LEDS - 1;
      div++;
    }
  }
  else // Ws28b12b_Segments was before initialized
  {
    for (uint16_t i = 0; i < (Segments > mSegments ? mSegments : Segments); i++)
    {
      ...
      SegmentsTmp[i].IdStop = div;
      if (SegmentsTmp[i].IdStop >= WS2812B_LEDS)
        Ws28b12b_Segments[i].IdStop = WS2812B_LEDS - 1;
      div++;
...
FX_STATUS WS2812BFX_NextMode(uint16_t Segment)
{
  ...
  if (Ws28b12b_Segments[Segment].ActualMode >= MODE_COUNT)
    Ws28b12b_Segments[mActualSegment].ActualMode = 0;
...
FX_STATUS WS2812BFX_PrevMode(uint16_t Segment)
{
  ...
  Ws28b12b_Segments[Segment].CounterModeStep = 0;
  Ws28b12b_Segments[Segment].ActualMode--;
  if (Ws28b12b_Segments[Segment].ActualMode == 0)
    Ws28b12b_Segments[mActualSegment].ActualMode = MODE_COUNT;
  Ws28b12b_Segments[Segment].mModeCallback = mMode[Ws28b12b_Segments[mActualSegment].ActualMode];
  return FX_OK;
}
...
FX_STATUS WS2812BFX_SetSegmentSize(uint16_t Segment, uint16_t Start, uint16_t Stop)
{
  if (Segment >= mSegments)
    return FX_ERROR;
  if (Start >= (WS2812B_LEDS - 1))
    return FX_ERROR;
  if (Stop >= (WS2812B_LEDS - 1))
    return FX_ERROR;
  if (Start > Stop)
    return FX_ERROR;
...
void mode_breath(void)
{
  ...
  if (Ws28b12b_Segments[mActualSegment].CounterModeStep > (512 - 15))
    Ws28b12b_Segments[mActualSegment].CounterModeStep = 15;
...
void mode_fade(void)
{
  ...
  if (Ws28b12b_Segments[mActualSegment].CounterModeStep > 511)
